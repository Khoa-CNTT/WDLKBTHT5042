...
const HealthChatbot = () => {
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            const response = await axios.post(`${backendURL}/api/ai-chatbot/symptoms`, { symptomData: input });
            ...
            const botMessage = {
                ...
                text: `
                    <div>
                        <p><b>Chuyên khoa gợi ý:</b> ${result.specialty}</p>
                        <p><b>Độ tin cậy:</b> ${result.confidence}%</p>
                        <p><b>Mức độ nghiêm trọng:</b> ${result.severity}</p>
            ...
            // Lọc danh sách bác sĩ dựa trên chuyên khoa
            if (result.specialty) {
                const filteredDoctors = doctors.filter(doctor => doctor.speciality === result.specialty);
            ...
            <div className="w-full h-[600px] lg:w-1/2 flex flex-col border border-gray-200 shadow-xl rounded-2xl bg-white">
                ...
                <div ref={messagesContainerRef}
                    ...
                    {messages.map((msg, index) => (
                        ...
                        >
                            <div
                                className={`max-w-[80%] p-4 rounded-2xl shadow-md relative ${
                                    msg.sender === 'user'
                ...
                <form onSubmit={handleSendMessage} className="flex p-4 border-t border-gray-200 bg-white rounded-b-2xl">
                    <input
                        ...
                        placeholder="Nhập triệu chứng của bạn (ví dụ: sốt, ho, đau bụng...)"
                    ...
                    <button
                        ...
                        className={`p-3 mx-3 rounded-full ${
                            isListening ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-200 hover:bg-gray-300'
